...
// function to perform forward pass on tensor via layer norm
forwardOutput forwardPassLayerNorm(torch::Tensor x, torch::Tensor gamma, torch::Tensor beta, float epsilon = 1e-5) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        // subtract mean from each feature in row (shift)
        for (int j = 0; j < dims; j++) {
            ptr_xmu[(i * dims) + j] = ptr_x[(i * dims) + j] - mu[i];  // subtract
        }

        // square each center feature to prep for variance
        for (int j = 0; j < dims; j++) {
            ptr_sq[(i * dims) + j] = ptr_xmu[(i * dims) + j] * ptr_xmu[(i * dims) + j];  // square
        }
    ...
    return {output, mu, var, std, ivar, cache_tensors, epsilon};  // return struct data types (output, floats, tensors)
...
int main() {
    ...
    float sum = 0.0f;
    float *ptr = result.output.data_ptr<float>();
    for (int i = 0; i < embedding_dims; i++) {
        sum += ptr[i];
    }
    float mean = sum / embedding_dims;
